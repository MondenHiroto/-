...
void CModel::Load(char *obj, char *mtl){
	...
	while (fgets(buf, sizeof(buf), fp) != NULL){
		...
		//文字列の比較
		//strcmp(文字列1,文字列2)
		//文字列1と文字列2が同じ時0、異なるとき0以外を返す
		//先頭がvの時ｍ頂点をvertexに追加する
		if (strcmp(str[0], "v") == 0){
			//可変長配列vertexに追加
		...
		//先頭がvtの時、uvに追加する
			...
			//可変長配列uvに追加
			//atof(文字列)　文字列からfloat型の値を返す
			uv.push_back(CVector(atof(str[1]), atof(str[2]), 0));
		}
	...
	//すべての法律の値をマテリアルの順番に保存する
	mpNormal = new float[mTriangles.size() * 9];
	//すべてのテクスチャマッピングの値をマテリアルの順番に保存する
